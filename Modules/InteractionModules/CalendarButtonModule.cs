    ...
    : ComponentInteractionModule<ButtonInteractionContext>
{
    ...
    public Task HandleToday()
    {
        return UpdateCalendarMessage(DateTime.UtcNow);
    }
    ...
    private async Task HandleNavigation(int dayModifier)
    {
                ...
                out var currentDate))
        {
            await RespondWithErrorAsync(
                "Invalid date format in the original message. Please try the `/calendar` command again.");
    ...
    private async Task UpdateCalendarMessage(DateTime targetDate)
    {
        ...
        var newEmbed = await calendarService.GetCalendar(targetDate);
